...
if __name__ == "__main__":
    # CUDA_VISIBLE_DEVICES=0 python demo.py --config config/vox-256.yaml --checkpoint checkpoints/vox.pth.tar --source_image ./assets/source.png --driving_video ./assets/driving.mp4
    ...
    parser.add_argument("--find_best_frame", default=False, dest="find_best_frame", action="store_true", 
    ...
    driving_video = []                  # 비디오를 fps로 잘라서 넣어줌 
    try:
        for im in reader:
    ...
    if opt.find_best_frame:
        ...
        predictions_forward = make_animation(source_image, driving_forward, inpainting_network, kp_detector, dense_motion_network, avd_network, device = device, mode = opt.mode)
        predictions_backward = make_animation(source_image, driving_backward, inpainting_network, kp_detector, dense_motion_network, avd_network, device = device, mode = opt.mode)
        predictions = predictions_backward[::-1] + predictions_forward[1:]
    else:
        predictions = make_animation(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network, device = device, mode = opt.mode)
